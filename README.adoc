...
And finally configure the *quality gates* on same configuration page:

image:sonar-quality-gates.png[]

TIP: Quality gates can `fail` a pipeline if it doesn't reach the quality gates conditions, feel free to test on http://localhost:9000/quality_gates/show/1[your sonar instance here^].

TIP: To change quality profiles you need to log in with `admin/admin` user.
...
NOTE: There is an (manual) aprovall step before "going to production". 
